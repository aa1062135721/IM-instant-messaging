	...
	export default {
		...
		methods: {
			...
			getUpdateInfo() {
				...
				this.$api.to_http('/app/getLatestAppVersion/'+key, {}, 'GET','', false,'application/json').then((res) => {
					console.log(res)
					uni.showToast({
						icon: 'none',
						title: res
					...
					// 获取当前设备对应的更新数据
					if (vm.platform == data.appTypeDisplay) {
						vm.update_info = {
							os: data.appTypeDisplay,
							version: data.versionNum,
							note: data.memo,
							download_url: data.downloadUrl, //下载路径
						}
					}
				...
				// uni.request({
				// 	url: 'http://47.111.10.36/api/app/getLatestAppVersion', //仅为示例，并非真实接口地址。
				// 	method: 'POST',
				// 	data:{
				// 		key:key
				// 	},
