...
const conversationModules = {
  ...
  mutations: {
    ...
    updateAllConversation (state, list) {
	  ...
	  console.log(state.allConversation, 'allConversation')
	  state.allConversation = state.allConversation.filter(res => res.type !== '@TIM#SYSTEM')
	  const result = state.allConversation.reduce((count, { unreadCount }) =>   count + unreadCount, 0)
	  if (result === 0) {
	    uni.removeTabBarBadge({ index: 1 })
	  } else {
	...
	// 更新好友申请列表
	listenFriendApplicationList(state, list) {
		console.log('store获取到的',list)
		state.unreadCount = list.unreadCount
		if (!list.friendApplicationList.length) return
		if (list.friendApplicationList && list.friendApplicationList.length) {
			state.friendApplicationList = list.friendApplicationList
		}
	},
    ...
    checkoutConversation (context, conversationID) {
      ...
      return wx.$app.getConversationProfile(conversationID)
        .then(({ data: { conversation } }) => {
		  ...
		  context.dispatch('totalUnreadCount')
          let name = ''
		  ...
		  if (conversation.type === 'C2C') {
			uni.navigateTo({ url: `/pages/ly-lq/chat/index?type=${conversation.type}&name=${name}&remark=${conversation.remark}&id=${conversation.userProfile.userID}` })  
		  } else if (conversation.type === 'GROUP') {
			uni.navigateTo({ url: `/pages/ly-lq/chat/index?type=${conversation.type}&name=${name}&id=${conversationID}` })  
		  }
